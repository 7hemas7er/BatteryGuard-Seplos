...
class SeplosBms : public PollingComponent, public seplos_modbus::SeplosModbusDevice {
 public:
  ...
  void set_protection_binary_sensor(binary_sensor::BinarySensor *protection_binary_sensor) {
    protection_binary_sensor_ = protection_binary_sensor;
  }
 ...
 protected:
  ...
  binary_sensor::BinarySensor *warning_binary_sensor_{nullptr};
  ...
  // Tracks which command (0x42 telemetry vs 0x44 telesignalization) was last
  // requested, so the shared response callback can pick the right decoder.
  uint8_t last_requested_function_{0x42};

  void publish_state_(binary_sensor::BinarySensor *binary_sensor, const bool &state);
  void publish_state_(sensor::Sensor *sensor, float value);
  void publish_state_(text_sensor::TextSensor *text_sensor, const std::string &state);
  void on_telemetry_data_(const std::vector<uint8_t> &data);
  void on_telesignalization_data_(const std::vector<uint8_t> &data);
